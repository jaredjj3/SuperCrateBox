...
class Player extends Moveable {
  ...
  handlePowerupCollision(powerupType, game) {
    switch(powerupType) {
      ...
      case 'electricShield':
        ...
        break;
      case 'nuke':
        const numberToRemove = game.enemies.length === 1 ?
          1 : Math.floor(game.enemies.length * 0.75);
  ...
  handleEnemyCollision(enemy, game) {
    ...
    if (isNotRecovering && this.electricShieldHitPoints > 0) {
      ...
      setTimeout(() => game.removeElectricShield(outerElectricShield.id), SHIELD_RECOVERY_TIME);
      this.lastHit = Date.now();
      return;
    }
